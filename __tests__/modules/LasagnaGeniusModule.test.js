import {
  ...
  addSecretIngredient,
  scaleRecipe,
} from "../../modules/challenges/LasagnaGeniusModule";
...
describe("performance tests for scaleRecipe", () => {
  test("scales a large recipe quickly", () => {
    const largeRecipe = {};
    for (let i = 0; i < 10000; i++) {
      largeRecipe[`ingredient${i}`] = i + 1;
    }

    const start = performance.now();
    scaleRecipe(largeRecipe, 4);
    const end = performance.now();

    const duration = end - start;
    console.log(`Scaling large recipe took ${duration} milliseconds`);
